...
export default function CoverLetterEditorPage() {
  ...
  // Restore synced resume from localStorage on mount
  useEffect(() => {
    if (id && session?.user) {
      const stored = localStorage.getItem(`cl-resume-sync-${id}`);
      if (stored) {
        try {
          const parsed = JSON.parse(stored);
          if (parsed && parsed._id) {
            // Fetch the full resume to get previewImage and content
            fetch(`/api/resumes/${parsed._id}`).then(r => r.json()).then(j => {
              if (j.data) {
                const r = j.data;
                const resumeData = {
                  _id: r._id,
                  ...
                  setData(prev => ({
                    ...
                    content: {
                      ...
                      personalInfo: {
                        ...
                        fullName: pi.fullName || prev.content.personalInfo.fullName,
                        email: pi.email || prev.content.personalInfo.email,
                        phone: pi.phone || prev.content.personalInfo.phone,
                        location: pi.location || prev.content.personalInfo.location,
                        professionalTitle: pi.professionalTitle || prev.content.personalInfo.professionalTitle,
                      },
                    },
                  }));
                }
  ...
  // Load Google Font on mount, after data loads, and when font family changes
  useEffect(()=>{
  ...
  const selectResume = (resume: ResumeListItem) => {
    ...
    try { localStorage.setItem(`cl-resume-sync-${id}`, JSON.stringify({ _id: resume._id })); } catch {}
    toast.success(`Synced with "${resume.title}"`);
  };

  const removeSync = () => {
    setSyncedResume(null);
    try { localStorage.removeItem(`cl-resume-sync-${id}`); } catch {}
    toast.success('Resume sync removed');
  };

  const handleAiApply = useCallback((html: string) => {
    un('content.body', html);
  ...
  const ResumeModal = () => {
    ...
    return (
      <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/50" onClick={()=>setShowResumeModal(false)}>
        <div className="bg-white rounded-2xl shadow-2xl w-[700px] max-h-[85vh] overflow-hidden flex flex-col" onClick={e=>e.stopPropagation()}>
          <div className="flex items-center justify-between px-6 py-4 border-b" style={{borderColor:'var(--app-border)'}}>
            <div className="flex items-center gap-3">
              <BookTemplate className="w-5 h-5" style={{color:'var(--app-primary)'}}/>
              <h2 className="text-lg font-bold" style={{color:'var(--app-text)'}}>Select Resume to Sync</h2>
            </div>
            <button onClick={()=>setShowResumeModal(false)} className="p-1.5 hover:bg-gray-100 rounded-lg transition-colors">
          ...
          <div className="flex-1 overflow-y-auto p-6">
            {loadingResumes ? (
              ...
              <div className="text-center py-20">
                <FileText className="w-12 h-12 mx-auto mb-3" style={{color:'var(--app-text-muted)'}}/>
                <p className="text-sm font-bold" style={{color:'var(--app-text-secondary)'}}>No resumes found</p>
                <p className="text-xs mt-1" style={{color:'var(--app-text-muted)'}}>Create a resume first to sync with this cover letter.</p>
              </div>
            ) : (
              <div className="grid grid-cols-2 gap-4">
                {resumes.map(resume => (
                  <button
                    ...
                    className={`relative rounded-xl border-2 overflow-hidden transition-all hover:shadow-lg hover:-translate-y-0.5 text-left ${
                      syncedResume?._id === resume._id ? 'border-[var(--app-primary)]' : 'border-gray-200'
                    }`}
                  >
                    {/* Preview Image */}
                    <div className="aspect-[3/4] bg-gray-50 flex items-center justify-center overflow-hidden">
                      {resume.previewImage ? (
                        <img src={resume.previewImage} alt={resume.title} className="w-full h-full object-cover" />
                      ) : (
                        <FileText className="w-12 h-12" style={{color:'var(--app-text-muted)'}}/>
                      )}
                    </div>
                    {/* Selected badge */}
                    {syncedResume?._id === resume._id && (
                      <div className="absolute top-2 right-2 bg-[var(--app-primary)] text-white text-[9px] font-black uppercase px-2 py-0.5 rounded-full">
                        Active
                      </div>
                    )}
                    {/* Title */}
                    <div className="p-3">
                      <p className="text-xs font-bold truncate" style={{color:'var(--app-text)'}}>{resume.title}</p>
                      {resume.content?.personalInfo?.fullName && (
                        <p className="text-[10px] mt-0.5 truncate" style={{color:'var(--app-text-muted)'}}>{resume.content.personalInfo.fullName}</p>
                      )}
                    </div>
                  </button>
                ))}
          ...
          {syncedResume && (
            <div className="flex items-center justify-between px-6 py-3 border-t" style={{borderColor:'var(--app-border)',background:'var(--app-bg-gray)'}}>
              <div className="flex items-center gap-2">
                <BookTemplate className="w-4 h-4" style={{color:'var(--app-primary)'}}/>
                <span className="text-xs font-medium" style={{color:'var(--app-text-secondary)'}}>Currently synced: <strong style={{color:'var(--app-text)'}}>{syncedResume.title}</strong></span>
              </div>
              <button onClick={removeSync} className="text-xs font-bold px-3 py-1 rounded-lg" style={{color:'var(--app-text-muted)'}}>Remove Sync</button>
            </div>
          )}
        </div>
      </div>
  ...
  return(<div className="flex flex-col h-screen font-sans" style={{background:'var(--app-bg)'}}>
    ...
    <header className="h-14 border-b flex items-center justify-between px-4 lg:px-6 z-50 shrink-0" style={{background:'var(--app-bg-card)',borderColor:'var(--app-border)'}}>
      <div className="flex items-center gap-3 min-w-0">
        <Link href="/cover-letters" className="flex items-center gap-2 px-3 py-1.5 rounded-lg hover:bg-gray-100 transition-all shrink-0"><ChevronRight className="h-4 w-4 rotate-180" style={{color:'var(--app-text-secondary)'}}/><span className="text-sm font-semibold hidden sm:inline" style={{color:'var(--app-text-secondary)'}}>Cover Letters</span></Link>
        <div className="w-px h-6 hidden sm:block" style={{background:'var(--app-border)'}}/>
        <input value={data.title} onChange={e=>un('title',e.target.value)} className="text-sm font-bold bg-transparent border-none focus:ring-0 focus:outline-none truncate max-w-[200px] lg:max-w-[300px]" style={{color:'var(--app-text)'}} placeholder="Click to edit letter name"/>
        <div className="flex items-center gap-1.5 ml-2 shrink-0"><div className={`w-2 h-2 rounded-full ${saving?'bg-orange-400 animate-pulse':'bg-emerald-400'}`}/><span className="text-[9px] font-black uppercase tracking-widest hidden sm:inline" style={{color:'var(--app-text-muted)'}}>{saving?'Saving...':'Saved'}</span></div>
      </div>
      <div className="flex items-center gap-2 shrink-0">
        {!isMobile&&<button onClick={()=>setActiveTab(activeTab==='write'?'customize':'write')} className="flex items-center gap-1.5 px-3 py-1.5 border text-xs font-bold rounded-xl transition-all" style={{borderColor:'var(--app-border)',color:'var(--app-text-secondary)'}}>{activeTab==='write'?<Settings className="h-3.5 w-3.5"/>:<FileText className="h-3.5 w-3.5"/>}<span>{activeTab==='write'?'Style':'Write'}</span></button>}
        <button onClick={exportPDF} className="flex items-center gap-1.5 px-4 py-1.5 text-xs font-bold rounded-xl shadow-sm" style={{background:'var(--app-primary)',color:'#fff'}}><Download className="h-3.5 w-3.5"/><span className="hidden sm:inline">Export PDF</span></button>
      </div>
    </header>
    <div className="flex flex-1 overflow-hidden relative">
      {isMobile?(<div className="flex-1 flex flex-col overflow-hidden">
        {showMobilePreview?(<div className="flex-1 overflow-auto p-2" style={{background:'#f1f5f9'}}><div style={{maxWidth:595,margin:'0 auto'}}>{renderPreview()}</div></div>):activeTab==='customize'?(<div className="flex-1 overflow-y-auto" style={{background:'var(--app-bg-gray)'}}><div className="p-3 space-y-2 pb-20">{renderPanel()}</div></div>):(<div className="flex-1 overflow-y-auto p-6" style={{background:'var(--app-bg)'}}>{renderEditor()}</div>)}
      </div>):(<div className="flex flex-1 overflow-hidden">
        <div className="w-[340px] xl:w-[380px] border-r overflow-y-auto shrink-0" style={{background:'var(--app-bg-gray)',borderColor:'var(--app-border)'}}>
          <div className="flex gap-1.5 px-3 pt-3 pb-1 shrink-0">
            <button onClick={()=>setActiveTab('write')} className={`flex-1 py-1.5 rounded-lg text-[10px] font-black uppercase tracking-widest text-center ${activeTab==='write'?'text-white':''}`} style={activeTab==='write'?{background:'var(--app-primary)'}:{color:'var(--app-text-secondary)'}}>Write</button>
            <button onClick={()=>setActiveTab('customize')} className={`flex-1 py-1.5 rounded-lg text-[10px] font-black uppercase tracking-widest text-center ${activeTab==='customize'?'text-white':''}`} style={activeTab==='customize'?{background:'var(--app-primary)'}:{color:'var(--app-text-secondary)'}}>Design</button>
          </div>
          <div className="p-3 space-y-2 pb-20">{renderPanel()}</div>
        </div>
        <div className="flex-1 overflow-y-auto p-8 lg:p-12" style={{background:'var(--app-bg)'}}>{renderEditor()}</div>
        <div className="w-[45%] xl:w-[42%] flex items-center justify-center p-8 overflow-hidden relative shrink-0" style={{background:'#f1f5f9'}}>{renderPreview()}</div>
      </div>)}
      <ResumeModal />
      <AiGenerateModal
        isOpen={showAiModal}
        ...
        bodyColor={d.bodyColor || '#374151'}
      />
      {isMobile&&(<div className="fixed inset-x-0 bottom-0 z-[50] flex items-center justify-around border-t px-2 py-2" style={{background:'rgba(15,23,42,0.96)',borderColor:'rgba(148,163,184,0.12)',paddingBottom:'calc(0.5rem + env(safe-area-inset-bottom))'}}>
        <button onClick={()=>{setActiveTab('write');setShowMobilePreview(false);}} className={`flex flex-col items-center gap-1 py-2 px-2 rounded-2xl ${activeTab==='write'&&!showMobilePreview?'text-[var(--app-primary)]':'text-gray-400'}`}><FileText className="w-5 h-5"/><span className="text-[9px] font-black uppercase">Write</span></button>
        <button onClick={()=>{setActiveTab('customize');setShowMobilePreview(false);}} className={`flex flex-col items-center gap-1 py-2 px-2 rounded-2xl ${activeTab==='customize'&&!showMobilePreview?'text-[var(--app-primary)]':'text-gray-400'}`}><Palette className="w-5 h-5"/><span className="text-[9px] font-black uppercase">Design</span></button>
    ...
    <SectionCard title="Resume Sync" icon={BookTemplate} defaultOpen={true}>
      {syncedResume ? (
        <div className="space-y-2">
          <div className="flex items-center gap-3 p-3 rounded-lg" style={{background:'var(--app-bg-gray)',border:'1px solid var(--app-border)'}}>
            <div className="w-12 h-12 rounded-lg overflow-hidden bg-white flex items-center justify-center shrink-0">
              {syncedResume.previewImage ? (
                <img src={syncedResume.previewImage} alt={syncedResume.title} className="w-full h-full object-cover" />
              ) : (
                <FileText className="w-6 h-6" style={{color:'var(--app-text-muted)'}}/>
              )}
            </div>
            <div className="flex-1 min-w-0">
              <p className="text-xs font-bold truncate" style={{color:'var(--app-text)'}}>{syncedResume.title}</p>
              {syncedResume.content?.personalInfo?.fullName && (
                <p className="text-[10px] truncate" style={{color:'var(--app-text-muted)'}}>{syncedResume.content.personalInfo.fullName}</p>
              )}
            </div>
            <button onClick={removeSync} className="text-[10px] font-bold px-2 py-1 rounded-lg" style={{color:'var(--app-text-muted)'}}>X</button>
          </div>
          <button onClick={openResumeModal} className="w-full py-2 text-[10px] font-bold rounded-lg" style={{background:'var(--app-bg-gray)',color:'var(--app-text-secondary)',border:'1px solid var(--app-border)'}}>Change Resume</button>
        </div>
      ) : (
        <button onClick={openResumeModal} className="w-full flex items-center justify-center gap-2 py-2.5 text-xs font-bold rounded-xl border-2 border-dashed transition-all" style={{borderColor:'var(--app-border)',color:'var(--app-text-secondary)'}}><BookTemplate className="w-4 h-4"/> Sync with Resume</button>
      )}
    </SectionCard>
  ...
  function renderEditor(){return(<div className="max-w-2xl mx-auto space-y-6 pb-32 lg:pb-10">
    {/* Sync with Resume */}
    <div className="flex items-center justify-between p-4 rounded-xl border shadow-sm cursor-pointer hover:shadow-md transition-all" style={{background:'var(--app-bg-card)',borderColor: syncedResume ? 'var(--app-primary)' : 'var(--app-border)'}} onClick={openResumeModal}>
      <div className="flex items-center gap-3">
        <BookTemplate className="w-5 h-5" style={{color: syncedResume ? 'var(--app-primary)' : 'var(--app-text-muted)'}}/>
        <div>
          <span className="text-sm font-bold" style={{color:'var(--app-text)'}}>
            {syncedResume ? `Synced: ${syncedResume.title}` : 'Sync with Resume'}
          </span>
          {syncedResume && syncedResume.content?.personalInfo?.fullName && (
            <p className="text-[10px] mt-0.5" style={{color:'var(--app-text-muted)'}}>{syncedResume.content.personalInfo.fullName}</p>
          )}
        </div>
      </div>
      {syncedResume && syncedResume.previewImage && (
        <div className="w-10 h-12 rounded overflow-hidden shrink-0 border" style={{borderColor:'var(--app-border)'}}>
          <img src={syncedResume.previewImage} alt="" className="w-full h-full object-cover" />
        </div>
      )}
    </div>
    ...
    <section className="rounded-xl p-4 shadow-sm border space-y-2" style={{background:'var(--app-bg-card)',borderColor:'var(--app-border)'}}>
      <div className="flex items-center justify-between">
        <h3 className="text-lg font-bold" style={{color:'var(--app-text)'}}>Body</h3>
        <button onClick={()=>setShowAiModal(true)} className="flex items-center gap-2 px-3 py-1.5 text-white text-xs font-bold rounded-lg transition-all" style={{background:'var(--app-primary)'}}><Sparkles className="h-3 w-3"/>AI Generate</button>
      </div>
      <CoverLetterBodyEditor
        content={data.content?.body || ''}
        onChange={(html) => un('content.body', html)}
        fontFamily={d.fontFamily || 'Inter'}
        fontSize={d.fontSize || 14}
        fontColor={d.bodyColor || '#374151'}
        lineHeight={d.lineHeight || 1.8}
        paragraphSpacing={d.paragraphSpacing || 12}
      />
    </section>
  ...
  function renderPreview(){
    const pd=d; const isModern=pd.layout==='modern'; const isMinimal=pd.layout==='minimal';
    const tAlign=isModern?'left':(pd.titleAlign||'center');
    const tSize=isMinimal?Math.max((pd.titleSize||16)-2,14):pd.titleSize||16;
    const pad=isMinimal?Math.max((pd.padding||48)-8,28):pd.padding||48;
    const gap=isMinimal?Math.max((pd.sectionGap||20)-6,8):pd.sectionGap||20;
    return(<div id="letter-preview" style={{width:'100%',maxWidth:595,aspectRatio:'1/1.414',overflow:'auto',background:pd.backgroundColor||'#ffffff',color:pd.textColor||'#111827',boxShadow:`0 4px 20px rgba(0,0,0,0.08),0 1px 3px rgba(0,0,0,0.05)${pd.borderStyle!=='none'&&(pd.borderWidth||0)>0?`,0 0 0 ${pd.borderWidth||0}px ${pd.primaryColor||'#ff4d7d'}`:''}`,padding:`${pad}px`,display:'flex',flexDirection:'column',gap,fontFamily:`"${pd.fontFamily||'Inter'}",sans-serif`,fontSize:pd.fontSize||12,lineHeight:pd.lineHeight||1.6,fontWeight:pd.fontWeight||400,letterSpacing:`${pd.letterSpacing||0}px`,borderRadius:`${pd.borderRadius||0}px`,borderTop:pd.topBorderAccent||isModern?`4px solid ${pd.primaryColor||'#ff4d7d'}`:'none'}}>
      {/* Title */}
      <div style={{textAlign:tAlign as any,fontWeight:700,fontSize:tSize,color:pd.titleColor||'#000',marginBottom:8,paddingBottom:(pd.titleStyle==='border-bottom'||pd.titleStyle==='double-line'||pd.titleStyle==='underline')?10:0,borderBottom:pd.titleStyle==='border-bottom'?`2px solid ${pd.primaryColor||'#000'}`:pd.titleStyle==='underline'?`1px solid ${pd.primaryColor||'#000'}`:'none',borderTop:pd.titleStyle==='overline'?`2px solid ${pd.primaryColor||'#000'}`:'none',background:pd.titleStyle==='background'?`${pd.primaryColor||'#ff4d7d'}15`:'transparent',padding:pd.titleStyle==='background'?'4px 8px':0,borderRadius:pd.titleStyle==='badge'?'4px':pd.titleStyle==='capsule'?'20px':0,paddingLeft:pd.titleStyle==='border-left'?8:pd.titleStyle==='capsule'?12:0,borderLeft:pd.titleStyle==='border-left'?`3px solid ${pd.primaryColor||'#000'}`:'none',paddingRight:pd.titleStyle==='border-right'?8:pd.titleStyle==='capsule'?12:0,borderRight:pd.titleStyle==='border-right'?`3px solid ${pd.primaryColor||'#000'}`:'none',display:pd.titleStyle==='double-side'||pd.titleStyle==='wave'?'flex':'block',alignItems:'center',boxShadow:pd.titleStyle==='shadow'?'2px 2px 4px rgba(0,0,0,0.1)':'none',backgroundImage:pd.titleStyle==='gradient'?`linear-gradient(90deg,${pd.primaryColor||'#ff4d7d'}22,transparent)`:'none',textDecoration:pd.titleStyle==='strikethrough'?'line-through':'none',...(pd.titleStyle==='double-line'?{borderBottom:'none'}:{})}}>
        {pd.titleStyle==='double-side'&&<span style={{flex:1,height:'1px',background:pd.primaryColor||'#000'}}/>}
        {pd.titleStyle==='dot'&&<span style={{marginRight:6,color:pd.primaryColor||'#000'}}>●</span>}
        {isModern?(data.content?.personalInfo?.fullName||data.title):data.title}
        {pd.titleStyle==='dot'&&<span style={{marginLeft:6,color:pd.primaryColor||'#000'}}>●</span>}
        {pd.titleStyle==='double-side'&&<span style={{flex:1,height:'1px',background:pd.primaryColor||'#000'}}/>}
        {pd.titleStyle==='wave'&&<span style={{flex:1,height:'1px',borderBottom:`1px wavy ${pd.primaryColor||'#000'}`}}/>}
      </div>
      {pd.titleStyle==='double-line'&&<div style={{marginTop:-12,marginBottom:8,textAlign:tAlign as any}}><div style={{width:tAlign==='center'?'60%':tAlign==='right'?'100%':'60%',marginLeft:tAlign==='right'?'auto':undefined,height:'2px',background:pd.primaryColor||'#000'}}/></div>}
      {isModern&&<div style={{height:'2px',background:pd.primaryColor||'#ff4d7d',width:'100%',marginBottom:4}}/>}
      {!pd.hideDate&&data.content?.date&&<div style={{textAlign:(pd.senderAlign||'right') as any,fontSize:11,color:'#6b7280'}}>{data.content.date}</div>}
      <div style={{marginTop:12,textAlign:(pd.senderAlign||'left') as any}}>
      ...
      {!pd.hideRecipient&&<div style={{marginTop:gap,textAlign:(pd.recipientAlign||'left') as any}}>
        <div style={{fontWeight:700,fontSize:12,color:pd.bodyColor||'#000'}}>To:</div>
        {data.content?.recipient?.name&&<div style={{fontSize:12,color:pd.bodyColor||'#374151'}}>{data.content.recipient.name}</div>}
        {data.content?.recipient?.company&&<div style={{fontSize:11,color:'#6b7280'}}>{data.content.recipient.company}</div>}
        {data.content?.recipient?.address&&<div style={{fontSize:11,color:'#6b7280'}}>{data.content.recipient.address}</div>}
      </div>}
      <div style={{flex:1,color:pd.bodyColor||'#374151',fontSize:pd.fontSize||12,lineHeight:pd.lineHeight||1.8,marginTop:gap,textAlign:(pd.bodyAlign||'left') as any,letterSpacing:`${pd.letterSpacing||0}px`,fontStyle:pd.italicBody?'italic':'normal',fontWeight:pd.boldBody?700:pd.fontWeight||400,textIndent:pd.firstLineIndent?`${pd.firstLineIndent}px`:undefined,fontFamily:`"${pd.fontFamily||'Inter'}",sans-serif`}}
        dangerouslySetInnerHTML={{__html: data.content?.body || ''}} />
      <style>{`#letter-preview p{margin:0 0 ${pd.paragraphSpacing||12}px 0;color:${pd.bodyColor||'#374151'};font-size:${pd.fontSize||12}px;line-height:${pd.lineHeight||1.8};text-align:${(pd.bodyAlign||'left')};font-family:"${pd.fontFamily||'Inter'}",sans-serif;letter-spacing:${pd.letterSpacing||0}px;}`}</style>
      {!pd.hideSignature&&<div style={{marginTop:32,borderTop:pd.signatureLine==='none'?'none':`1px ${pd.signatureLine==='dashed'?'dashed':pd.signatureLine==='dot'?'dotted':'solid'} #e5e7eb`,paddingTop:16}}>
